...
import sys
from utils import *
from loralayer import Lora_Embedding, Lora_Linear
...
class FeedForward(nn.Module):
  def __init__(self, args)->None:
    ...
    self.w1 = Lora_Linear(args.dim, hidden, args.r, args.lora_alpha, args.lora_dropout, bias = False, dtype = torch.float16)
    self.w2 = Lora_Linear(hidden, args.dim, args.r, args.lora_alpha, args.lora_dropout, bias = False, dtype = torch.float16)
    self.w3 = Lora_Linear(args.dim, hidden, args.r, args.lora_alpha, args.lora_dropout, bias = False, dtype = torch.float16)
...
class Lora_Transformer(nn.Module):
  def __init__(self, args)->None:
    ...
    self.output = Lora_Linear(args.dim, args.vocab_size, args.r, args.lora_alpha, args.lora_dropout, bias = False, dtype = torch.float16)
