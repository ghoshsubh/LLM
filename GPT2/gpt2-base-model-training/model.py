...
class GPTModel(nn.Module):
  ...
  @torch.no_grad()
  def generate(self, idx, new_token_size, time_step, temperature = 1.0, top_k = None, do_sample = True):
    for _ in range(new_token_size):
      idx_cond = idx if idx.shape[-1] < time_step else idx[:, -time_step:]
      
      logits, _ = self(idx_cond, device = 'cuda')
      
      logits = logits[:, -1, :] / temperature #Take out the logits of the final step
      ...
      if do_sample:
        ...
        _, idx_next = torch.topk(proba, k = 1, dim = -1)      
